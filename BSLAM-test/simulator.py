...
class lidarSimulator:
    def __init__(self, startPos, startAngle, numScans, maxRange, noisePct, lines):
        self.position = startPos
        self.angle = startAngle
        self.numScans = numScans
        self.maxRange = maxRange
        self.noisePct = noisePct
        self.lines = lines
    
    def translate(self, translation):
        self.position = Point(self.position.x + translation.x, self.position.y + translation.y)
    ...
    def simLidar(self):
        self.scan = []
        self.points = []
        self.scan,self.points = lidar(self.position, self.angle, self.numScans, self.maxRange, self.lines)
        self.scan = addNoise(self.scan, self.noisePct)
    ...
    def plotScan(self, name):
        fig = plt.figure(1)
        plt.plot(self.scan, 'b.')
        plt.xlabel('scan number')
        plt.ylabel('distance')

        #plt.figure(2)
        
        #plt.plot([p.x for p in self.points], [p.y for p in self.points], 'b.')
        #plt.grid(True)
